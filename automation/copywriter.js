...
- طرح رو توصیف نکن؛ خودش تو عکس پیداست. اسم نبر که چی کشیده شده (آدم، لباس، پوشش، کلاه، شیء، حیوان، نماد و …) و نگو «این طرح یه … داره» — تشخیصِ تصویر معمولاً اشتباهه، مخصوصاً برایِ چیزهای فرهنگیِ ایرانی. مرجع‌های مطمئن فقط «اسمِ محصول در سایت» و «یادداشتِ صاحبِ برند»ـن (اگه داده شد). ریشه‌یِ طرح (مالِ کدوم شاعر/خواننده/آهنگ/فیلمه) رو حدس نزن؛ فقط اگه یادداشت گفته بود، همونو بگو — می‌تونی باهاش بازی کنی.
- به‌جاش از حس‌وحال و نگرش حرف بزن: به چه آدمی میاد، کجا و کِی پوشیدنش حال می‌ده، چی به دیگرون می‌گه، برایِ کی هدیه‌ی خوبیه. کلی‌گویی‌ای که برای هر لباسی درست باشه هم ممنوعه — حس و مخاطب باید به همین محصول بخوره.
...
function fallbackCopy({ label, facts }) {
  const heads = [
    ...
    `${label} امروزمون اینه`,
    `اینو ببین و بگو بهت نمیاد`,
    `یه ${label} که تو جمع گم نمی‌شه`,
    `هدیه‌ای که تکراری درنمیاد`,
  ];
  const closers = ["لینکِ خرید تو بیوئه.", "برای سایز و رنگ دایرکت بده.", "از سایتِ میزطوری سفارش بده."];
  const pick = (a) => a[Math.floor(Math.random() * a.length)];
  const headline = pick(heads);
  const middle = pick([
    "طرح‌هامون فقط تو میزطوری پیدا می‌شن.",
    facts?.colors?.length ? `${facts.colors.length} تا رنگ داره؛ با هر چی بپوشی ست می‌شه.` : "کلی رنگ داره؛ با هر چی بپوشی ست می‌شه.",
...
${BRAND_POINTS}
چیزی که رویِ طرح‌ها کشیده شده رو توصیف نکن و از اسمِ محصول چیزی حدس نزن (مثلاً نگو مالِ کدوم شاعر/خواننده/آهنگه)؛ فقط اگه «یادداشتِ صاحبِ برند» داشت، می‌تونی همونو بگی؛ از حس‌وحال و آدمی که بهش میاد بگو.
...
- caption: ۲ تا ۳ خط: یه جمله دربارهِ حس‌وحال یا آدمِ مناسبِ هر کدوم (بدونِ توصیفِ تصویرِ طرح)، بعد بخواد تو کامنت بنویسه الف یا ب، و اینکه لینکِ هر دو تو سایته.`,
      9
    );
  }
...
- caption: ۲ تا ۴ خطِ کوتاه (هر خط یه سطر). اسمِ یکی دو تا از طرح‌ها رو همون‌طور که تو سایته بیار و بگو به چه آدمی میان (بدونِ توصیفِ تصویرِ طرح)، و آخرش یه کارِ مشخص برایِ خرید.`,
    8
  );
}
